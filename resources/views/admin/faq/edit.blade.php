        ...
        <form action="{{ route('admin.faq.update', $faq->id) }}" method="POST">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="question" value="{{ old('question', $faq->question) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('question') border-red-500 @enderror">
                    @error('question')
                ...
                <div>
                    ...
                    <textarea name="answer" rows="5" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('answer') border-red-500 @enderror">{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $faq->order) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('order') border-red-500 @enderror">
                        @error('order')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
